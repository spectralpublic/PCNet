...
def train(train_loader, model, criterion, optimizer_conv,scheduler_conv, optimizer_fc, scheduler_fc, epoch, step):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # compute output
        logit1_self, logit2_self, labels1, labels2, logit1_mut, logit2_mut = model(input_var, target_var, flag='train')
        batch_size = logit1_self.shape[0]
        labels1 = labels1.to(device)
        labels2 = labels2.to(device)

        self_logits = torch.zeros(2 * batch_size, 45).to(device)
        other_logits = torch.zeros(2 * batch_size, 45).to(device)
        self_logits[:batch_size] = logit1_self
        self_logits[batch_size:] = logit2_self
        other_logits[:batch_size] = logit1_mut
        other_logits[batch_size:] = logit2_mut
        ...
        # rank_loss = rank_criterion(other_scores, self_scores, flag)
        ...
        if i== len(train_loader) - 2:
            ...
            with open("test.txt", "a") as f:
                f.write('* Prec@1 {top1:.3f}  Best {best:.3f}\n'.format(top1=prec1, best=best_prec1))
        step = step +1
    return step
